...
<table>
  ...
  <tr>
   ...
   <tr>
    <td><code>badv</code></td>
    <td>string array</td>
    <td>Array of advertiser domains (e.g., “ford.com”) that are blocked from bidding on this deal. This list augments the blocked advertisers specified on the <code>BidRequest</code>. <code>badv</code> and <code>wadomain</code> are mutually exclusive and only one of them should be present on a given deal. If they are both specified, `wadomain` takes precedence as an allow list implicitly blocks any advertiser domains not in it. How should `BidRequest.badv` and `Deal.wadomain` be reconciled?</td>
   </tr>
   <tr>
